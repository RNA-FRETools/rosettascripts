...
def parseCmd(version):
    ...
    parser.add_argument(
        "-e",
        "--edit",
        help="'oldResi>newResi' (use \",\" to separate individual residues and \"-\" for residue ranges; e.g. '2-4,5>A:6-8,9')",
        required=True,
    )
    ...
    def reassign(oC, oR, nC, nR):
        ...
        if oC == nC:
            ...
            if nC not in chainIDs:
                if nC != "?":
                    print(
                        "-> Note: chain '{}' is not present in '{}'. Adding it to the structure".format(
                            nC, structure_id
                        )
                    )
    ...
    for i, ((oC_ori, oR_ori), (nC_ori, nR_ori)) in enumerate(zip(residues["old"], residues["new"])):
        if isPres[i]:
            ...
            try:
                ...
                print(
                    "-> Error: residue '{}:{}' could not be assigned to '{}:{}' since that residue already exists. No output is generated".format(
                        oC_ori, oR_ori, nC_ori, nR_ori
                    )
                )
                sys.exit()
...
def writePDB(structure_id, structure, inplace, outputfile):
    ...
    print(
        "=> Successfully renumbered (ID: {}) and written to directory ('{}{}.pdb').".format(
            structure_id, structure_id, renumber
        )
    )
    return
...
def main():
    pdbfile, editline, inplace, outputfile = parseCmd(str(metadata["Version"]))
